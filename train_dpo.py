...
class TrainDpo:
    def __init__(self):
        ...
        self.data_loader = DataLoader(dataset, batch_size=self.config.batch_size, shuffle=True, collate_fn=dataset.collate_fn)
        self.dpo = DPO(self.model, self.model_opt, self.config)

    def train_dpo(self):
        for _epoch in range(self.config.epochs):
            for batch_data in self.data_loader:
                ref_logits = self.reference_model.generate_logits(batch_data["inputs_ids"].to(self.config.device).T)  # 获得参考模型的logit
                self.dpo.train(
                    batch_data["inputs_ids"].to(self.config.device),
                    batch_data["inputs_masks"].to(self.config.device),
                    ref_logits.permute(1, 0, 2),
