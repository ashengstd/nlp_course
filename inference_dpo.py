...
from dpo_model.config import Config, LoraArguments
...
class LoraDPOModel(PeftModel):
    ...
    def predict(model_inputs, model, tokenizer):
        generated_ids = model.generate(model_inputs.input_ids, max_new_tokens=512, num_beams=1, do_sample=False)
        generated_ids = [
...
if __name__ == "__main__":
    ...
    lora_dpo_response, raw_response = lora_dpo_model(
        "操作说明：为一个关于回收的网站设计一个标志。", "以下是描述任务的说明。编写适当地完成请求的响应。"
    )
